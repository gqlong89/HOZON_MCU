...
# 10 "../Sources/mid/timer/timer.c" 2
...
void FTM_HAL_SetChnInputCaptureFilter(FTM_Type * const ftmBase,
                                      uint8_t channel,
                                      uint8_t value);
# 11 "../Sources/mid/timer/timer.c" 2
...
# 12 "../Sources/mid/timer/timer.c" 2
...
# 13 "../Sources/mid/timer/timer.c" 2
...
# 5 "C:/Users/INTEST/Desktop/MCU(2)/MCU/Sources/include/softrtc.h" 2
...
# 14 "../Sources/mid/timer/timer.c" 2
...
# 16 "../Sources/mid/timer/timer.c" 2
...
# 17 "../Sources/mid/timer/timer.c" 2
...
static void timer_irq_handler(TIMER_CTX *ctx, TIMER_ATTR *attr)
{
    ...
    for (handle = 0; handle < ctx->create_cnt; handle++)
    {
        ...
        if (0xFFFFFFFF == ctx->cur_val[handle] || TIMER_STOP == ctx->status[handle])
        {
            continue;
        }
...
static void timer_init(TIMER_TYPE type)
{
    ...
    {
        ctx->total_val[handle] = 0xFFFFFFFF;
        ctx->cur_val[handle] = 0xFFFFFFFF;
    ...
    if (TIMER_LOW_PRE == type)
    {
        ...
        timer_cfg.finalValue = 6250U;
    }
    else
    {
        timer_cfg.finalValue = 125U;
    }
...
static int8_t timer_create(TIMER_TYPE type, timeout_handler timeout)
{
    ...
    if ((ctx->create_cnt >= 100) || (((void *)0) == timeout))
    {
        return -1;
    }
...
static int8_t timer_start(TIMER_TYPE type, int8_t handle, uint32_t val)
{
    ...
    if ((handle < 0) || (handle >= ctx->create_cnt) || (0xFFFFFFFF == val))
    {
        return -1;
    }
...
static int8_t timer_stop(TIMER_TYPE type, int8_t handle)
{
    ...
    if ((0xFFFFFFFF == ctx->total_val[handle]) || (0 == ctx->run_cnt))
    {
        INT_SYS_DisableIRQ(attr->irq);
...
{
    CAN_MSG_BUF *can_buf = ((void *)0);
    uint32_t tick;


    FTM_HAL_ClearTimerOverflow(g_ftmBase[3U]);

    while (!SysTick_get_tag(&tick))
    {
        if ((can_buf = (CAN_MSG_BUF *)ipc_can_handle(SPI_GET_FRAME)) != ((void *)0))
        {
...
void ctimer_init(void)
{
    ...
    FTM_DRV_Init(3U, &init_cfg, &ctmState);
    INT_SYS_InstallHandler(FTM3_Ovf_Reload_IRQn, ctimer_irt_handler, (isr_t *) 0);
    FTM_DRV_InitCounter(3U, &timer_cfg);
}

void ctimer_start(void)
{
    FTM_DRV_CounterStart(3U);
}

void ctimer_stop(void)
{
    FTM_DRV_CounterStop(3U);
}
...
void timer_sleep(void)
{
    ...
    INT_SYS_DisableIRQ(FTM3_Ovf_Reload_IRQn);
}
