# 1 "../Sources/app/tp/proxy/Proxy.c"
...
# 2 "../Sources/app/tp/proxy/Proxy.c" 2
...
void uds_data_indication(void *uds_para, uint32_t u32AI, uint16_t u32PDU_DLC, uint8_t *pu8_PDUData,
                         uint8_t eNResult);
# 3 "../Sources/app/tp/proxy/Proxy.c" 2
...
# 4 "../Sources/app/tp/proxy/Proxy.c" 2
...
# 5 "../Sources/app/tp/proxy/Proxy.c" 2
...
# 6 "../Sources/app/tp/proxy/Proxy.c" 2
...
# 7 "../Sources/app/tp/proxy/Proxy.c" 2
...
# 8 "../Sources/app/tp/proxy/Proxy.c" 2
...
# 10 "../Sources/app/tp/proxy/Proxy.c" 2
...
void proxy_upload(uint8_t *pdata , uint16_t length, uint8_t type)
{
    ...
    {
        data_retry.flag ++;
        do { if( ( !g_xModemCommEnable ) && (PRINTK_LEVEL_DEBUG) >= kernelGetDebugLevel() ) { if(PRINTK_LEVEL_DEBUG == PRINTK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" Kernel>"); printf("%s() ", __func__); printf( "data covered times :%d", data_retry.flag ); printf("\r\n"); if(PRINTK_LEVEL_DEBUG == PRINTK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0);
    }
    ...
    kernelSetTaskEvent(task_uds_id, (1<<23));
...
void uds_result_confirm(void *uds_para, uint32_t u32AI, uint8_t eNResult)
{
    ...
    if ((N_DRIVER_ERR != eNResult) && (N_OK != eNResult) && (N_ERROR != eNResult))
    {
        return;
    }

    uds_ask_msg_head.CanID = u32AI;
    uds_ask_msg_head.MsgID = 0xF5;
...
static void uds_param_set_ask(void)
{
    ...
    uds_ask_set_head.MsgID = 0xF7;
    uds_ask_set_head.result = 1;
