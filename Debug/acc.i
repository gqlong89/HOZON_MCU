...
int acc_device_init(void)
{
    ...
    if (acc_init_cnt > 3)
    {
        driverI2CInit();
        acc_init_cnt = 0;
        do { if( ( !g_xModemCommEnable ) && (PRINTK_LEVEL_ERROR) >= kernelGetDebugLevel() ) { if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" Kernel>"); printf("%s() ", __func__); printf( "REINIT I2C DRIVER" ); printf("\r\n"); if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0);
    }
    ...
    else
    {
        ...
        acc_init_cnt++;
        ret = -1;
        do { if( ( !g_xModemCommEnable ) && (PRINTK_LEVEL_ERROR) >= kernelGetDebugLevel() ) { if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" Kernel>"); printf("%s() ", __func__); printf( "unknow acc type" ); printf("\r\n"); if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0);
    }
...
void acc_check(void)
{
    ...
    if (err_cnt > 3)
    {
        do { if( ( !g_xModemCommEnable ) && (PRINTK_LEVEL_ERROR) >= kernelGetDebugLevel() ) { if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" Kernel>"); printf("%s() ", __func__); printf( "read error max, reinit acc" ); printf("\r\n"); if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0);
        acc_type = ACC_UNKNOWN;
        err_cnt = 0;
    }
}
