...
# 7 "../Sources/mid/kernel/debug.c" 2
...
# 8 "../Sources/mid/kernel/debug.c" 2
...
# 9 "../Sources/mid/kernel/debug.c" 2
...
# 10 "../Sources/mid/kernel/debug.c" 2
...
# 11 "../Sources/mid/kernel/debug.c" 2

# 1 "C:/Users/INTEST/Desktop/MCU(2)/MCU/SDK/platform/devices/S32K144/include/S32K144.h" 1
# 168 "C:/Users/INTEST/Desktop/MCU(2)/MCU/SDK/platform/devices/S32K144/include/S32K144.h"
extern unsigned int irq_cnt[20];
# 13 "../Sources/mid/kernel/debug.c" 2
...
static IRQ_PRIOR_MAP irq_pri[] =
{
    ...
    { CAN0_Error_IRQn, 4 },
    { CAN0_Wake_Up_IRQn, 4 },
    { CAN0_ORed_0_15_MB_IRQn, 4 },
    { CAN0_ORed_16_31_MB_IRQn, 4 },
    { CAN1_ORed_IRQn, 4 },
    { CAN1_Error_IRQn, 4 },
    { CAN1_ORed_0_15_MB_IRQn, 4 },
    { CAN2_ORed_IRQn, 4 },
    { CAN2_Error_IRQn, 4 },
    { CAN2_ORed_0_15_MB_IRQn, 4 },
    { LPSPI0_IRQn, 4 },
    { LPSPI1_IRQn, 3 },
    { LPSPI2_IRQn, 3 },
    { PORTA_IRQn, 4 },
    ...
    { WDOG_EWM_IRQn, 6 },
    { FTM1_Ovf_Reload_IRQn, 7 },
    { FTM2_Ovf_Reload_IRQn, 7 },
    { FTM3_Ovf_Reload_IRQn, 4 },
};
...
char *kernelGetVersion(void)
{
    return ("MCU_J311HZCOMM003[A-11]" "\\" "Dec  4 2019" "\\" "11:08:06");
}


char *kernelGetVerNo(void)
{
    return "MCU_J311HZCOMM003[A-11]";
}

char *kernelGetCurTimeStr(void)
{
    static char timeStr[24];
# 112 "../Sources/mid/kernel/debug.c"
    return timeStr;
}
...
#define BIN2CHAR(ch) (((ch) > ' ' && (ch) <= '~') ? (ch) : '.')
    ...
    for (i = 0; i < len1; i += 16)
    {
        ...
        for (j = 0; j < 16; j++)
        {
            sprintf(ibuf, "%c", (((ptr[j]) > ' ' && (ptr[j]) <= '~') ? (ptr[j]) : '.'));
            ibuf++;
        }
    ...
    for (i = 0; i < len2; i++)
    {
        sprintf(ibuf, "%c", (((ptr[i]) > ' ' && (ptr[i]) <= '~') ? (ptr[i]) : '.'));
        ibuf += 1;
    }
...
int showirq(unsigned int argc, unsigned char **argv)
{
    ...
    if (argc == 2 && argv[0][0] == '!')
    {
        do { if( !g_xModemCommEnable ) { printf( " %10s - show irq cnt.\r\n", argv[1] ); if( shellRedirectBufSize > 1 ) { redirectLen = snprintf( (char *)shellRedirectBuf, shellRedirectBufSize , " %10s - show irq cnt.\r\n", argv[1] ); if ( (redirectLen > 0) && ( redirectLen < shellRedirectBufSize ) ) { shellRedirectBufSize -= redirectLen; shellRedirectBuf += redirectLen; } else { shellRedirectBufSize = 0; } } } } while (0);
        return 0;
    }

    do { if( !g_xModemCommEnable ) { printf( " ****IRQ Count Begin****\r\n" ); if( shellRedirectBufSize > 1 ) { redirectLen = snprintf( (char *)shellRedirectBuf, shellRedirectBufSize , " ****IRQ Count Begin****\r\n" ); if ( (redirectLen > 0) && ( redirectLen < shellRedirectBufSize ) ) { shellRedirectBufSize -= redirectLen; shellRedirectBuf += redirectLen; } else { shellRedirectBufSize = 0; } } } } while (0);

    for (i = 0; i < sizeof(irq_cnt) / sizeof(irq_cnt[0]); i++)
    {
        if (irq_cnt[i])
        {
            do { if( !g_xModemCommEnable ) { printf( " irq[%03u]=[%010u,%010u],[%u]/Sec \r\n", i, irq_cnt[i], irq_last[i], (irq_cnt[i] - irq_last[i]) * 100 / (SysTick_GetTickCount() - last_tick) ); if( shellRedirectBufSize > 1 ) { redirectLen = snprintf( (char *)shellRedirectBuf, shellRedirectBufSize , " irq[%03u]=[%010u,%010u],[%u]/Sec \r\n", i, irq_cnt[i], irq_last[i], (irq_cnt[i] - irq_last[i]) * 100 / (SysTick_GetTickCount() - last_tick) ); if ( (redirectLen > 0) && ( redirectLen < shellRedirectBufSize ) ) { shellRedirectBufSize -= redirectLen; shellRedirectBuf += redirectLen; } else { shellRedirectBufSize = 0; } } } } while (0)
                                                                                                ;
    ...
    do { if( !g_xModemCommEnable ) { printf( " tick = %d, cnt = %d\r\n", last_tick, SysTick_GetCurCounter() ); if( shellRedirectBufSize > 1 ) { redirectLen = snprintf( (char *)shellRedirectBuf, shellRedirectBufSize , " tick = %d, cnt = %d\r\n", last_tick, SysTick_GetCurCounter() ); if ( (redirectLen > 0) && ( redirectLen < shellRedirectBufSize ) ) { shellRedirectBufSize -= redirectLen; shellRedirectBuf += redirectLen; } else { shellRedirectBufSize = 0; } } } } while (0);
    ipc_list_show();
    do { if( !g_xModemCommEnable ) { printf( " ****IRQ Count End****\r\n" ); if( shellRedirectBufSize > 1 ) { redirectLen = snprintf( (char *)shellRedirectBuf, shellRedirectBufSize , " ****IRQ Count End****\r\n" ); if ( (redirectLen > 0) && ( redirectLen < shellRedirectBufSize ) ) { shellRedirectBufSize -= redirectLen; shellRedirectBuf += redirectLen; } else { shellRedirectBufSize = 0; } } } } while (0);
    return 1;
}
