# 1 "../Sources/mid/can_spi/mcp2515.c"
...
# 56 "C:/Users/INTEST/Desktop/MCU(2)/MCU/SDK/platform/hal/inc/lpspi_hal.h" 2
...
# 9 "../Sources/mid/can_spi/mcp2515.c" 2
...
# 10 "../Sources/mid/can_spi/mcp2515.c" 2
# 1 "C:/Users/INTEST/Desktop/MCU(2)/MCU/SDK/platform/drivers/inc/lpspi_shared_function.h" 1
# 11 "../Sources/mid/can_spi/mcp2515.c" 2
# 1 "C:/Users/INTEST/Desktop/MCU(2)/MCU/Sources/mid/can_spi/can_spi.h" 1
...
_Bool Get_send_sta(uint32_t instance, uint8_t num, uint8_t *perr);

_Bool spican_deinit(uint32_t instance);
void can_spi_test(void);
# 12 "../Sources/mid/can_spi/mcp2515.c" 2
# 1 "C:/Users/INTEST/Desktop/MCU(2)/MCU/Sources/mid/can_spi/mcp2515.h" 1
...
#define SPI_RTS_0 (1<<0)
#define SPI_RTS_1 (1<<1)
#define SPI_RTS_2 (1<<2)

#define SPI_CMD32_PACK(B3,B2,B1,B0) ( (uint32_t)( ( ((uint32_t)B3&0xFF)<<24 ) | ( ( (uint32_t)B2&0xFF) <<16 ) | ( ( (uint32_t)B1&0xFF) <<8 ) | (uint32_t)B0&0xFF) )




#define SPI_CMD24_PACK(B2,B1,B0) ( (uint32_t)( ( (uint32_t)0x0 <<24 ) | ( ( (uint32_t)B2&0xFF)<<16 ) | ( ( (uint32_t)B1&0xFF)<<8 ) | (uint32_t)B0&0xFF) )




#define SPI_CMD16_PACK(B1,B0) ( (uint32_t)( ( (uint32_t)0x0 <<24 ) | ( (uint32_t)0x0 <<16 ) | ( ((uint32_t)B1&0xFF)<<8 ) | (uint32_t)B0&0xFF) )




void mcp2515_init(uint32_t instance);
uint8_t mcp2515_write_register(uint32_t instance, uint8_t adress, uint8_t data);
uint8_t mcp2515_read_register(uint32_t instance, uint8_t adress);
void mcp2515_bit_modify(uint32_t instance, uint8_t adress, uint8_t mask, uint8_t data);
void mcp2515_reset(uint32_t instance);

void mcp2515_read_registers(uint32_t instance, uint8_t addr, uint8_t *prx, uint8_t len);
_Bool mcp2515_read_buff(uint32_t instance, uint8_t *prx, uint8_t addr, uint8_t len);
_Bool mcp2515_write_buff(uint32_t instance, uint8_t *ptx, uint8_t addr, uint8_t len);
void Send_start(uint32_t instance, uint8_t num);
# 13 "../Sources/mid/can_spi/mcp2515.c" 2
# 1 "c:\\nxp\\s32ds_arm_v1.3\\cross_tools\\gcc-arm-none-eabi-4_9\\lib\\gcc\\arm-none-eabi\\4.9.3\\include\\stddef.h" 1 3 4
# 14 "../Sources/mid/can_spi/mcp2515.c" 2
...
{
    return ((uint8_t)LPSPI_DRV_MasterTransferBlocking(instance, tx, rx, len, (10)));
...
void mcp2515_init(uint32_t instance)
{
    ...
    if ((1U) == instance)
    {
        LPSPI_DRV_MasterInit((1U), &lpspiCan1State, &lpspican_MasterConfig);
        LPSPI_HAL_Disable(g_lpspiBase[instance]);
        LPSPI_HAL_SetPinConfigMode(g_lpspiBase[instance], LPSPI_SDI_OUT_SDO_IN,
                                   LPSPI_DATA_OUT_RETAINED, 1);
        LPSPI_DRV_MasterSetDelay((1U), 1, 1,
                                 1);
    ...
    {
        LPSPI_DRV_MasterInit((2U), &lpspiCan2State, &lpspican_MasterConfig);
        LPSPI_HAL_Disable(g_lpspiBase[instance]);
        LPSPI_HAL_SetPinConfigMode(g_lpspiBase[instance], LPSPI_SDI_OUT_SDO_IN,
                                   LPSPI_DATA_OUT_RETAINED, 1);
        LPSPI_DRV_MasterSetDelay((2U), 1, 1, 1);
        LPSPI_HAL_Enable(g_lpspiBase[instance]);
    }
    else
    {
...
uint8_t mcp2515_write_register(uint32_t instance, uint8_t addr, uint8_t data)
{
    ...
    cmd[1] = addr;
    cmd[2] = data;
    return can_spi_master_transmit(instance, cmd, ((void *)0), 3);
}
...
void mcp2515_write_registers(uint32_t instance, uint8_t addr, uint8_t *ptx, uint8_t len)
{
    ptx[0] = 0x02;
    ptx[1] = addr;
    can_spi_master_transmit(instance, ptx, ((void *)0), len);
}
...
void mcp2515_bit_modify(uint32_t instance, uint8_t addr, uint8_t mask, uint8_t data)
{
    ...
    cmd[2] = mask;
    cmd[3] = data;
    can_spi_master_transmit(instance, cmd, ((void *)0), 4);
}

void mcp2515_reset(uint32_t instance)
{
    uint8_t cmd = 0xC0;
    can_spi_master_transmit(instance, &cmd, ((void *)0), 1);
}
