# 1 "../Sources/app/scom/scom_can_msg.c"
...
# 9 "../Sources/app/scom/scom_can_msg.c" 2
...
# 10 "../Sources/app/scom/scom_can_msg.c" 2
...
# 12 "../Sources/app/scom/scom_can_msg.c" 2
...
# 13 "../Sources/app/scom/scom_can_msg.c" 2
...
# 14 "../Sources/app/scom/scom_can_msg.c" 2
...
# 15 "../Sources/app/scom/scom_can_msg.c" 2
...
# 16 "../Sources/app/scom/scom_can_msg.c" 2
...
static void set_can_report(unsigned char *msg, unsigned int len)
{
    ...
    for (i = 0; i < no; i++)
    {
        ...
        do { if( ( !g_xModemCommEnable ) && (TASK_LEVEL_LOG) >= kernelGetTaskLevel() ) { if(TASK_LEVEL_LOG == TASK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" %s>", kernelGetTaskName()); printf("%s() ", __func__); printf( "rtype:0x%x,ftype:0x%x,port:%u,dlc:%u,id:0x%08x:val:%u,times:%u!", cfg->rtype, cfg->ftype, cfg->port, cfg->dlc, cfg->id, cfg->val, cfg->times ); printf("\r\n"); if(TASK_LEVEL_LOG == TASK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0)
                                                                                              ;
...
void scom_can_proc(SCOM_TL_MSG_HDR *hdr, unsigned char *body)
{
    switch (hdr->msg_type)
    {
        ...
        default:
            do { if( ( !g_xModemCommEnable ) && (TASK_LEVEL_ERROR) >= kernelGetTaskLevel() ) { if(TASK_LEVEL_ERROR == TASK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" %s>", kernelGetTaskName()); printf("%s() ", __func__); printf( "unknown can msgid=%u!", hdr->msg_type ); printf("\r\n"); if(TASK_LEVEL_ERROR == TASK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0);
            break;
    }
}
