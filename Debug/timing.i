...
# 7 "../Sources/mid/kernel/timing.c" 2
...
# 8 "../Sources/mid/kernel/timing.c" 2
...
# 9 "../Sources/mid/kernel/timing.c" 2
...
# 10 "../Sources/mid/kernel/timing.c" 2
...
# 11 "../Sources/mid/kernel/timing.c" 2
...
# 12 "../Sources/mid/kernel/timing.c" 2

static volatile timerStruct timerList[(15)];


void kernelInitTimer(void)
{
    do { if( ( !g_xModemCommEnable ) && (PRINTK_LEVEL_LOG) >= kernelGetDebugLevel() ) { if(PRINTK_LEVEL_LOG == PRINTK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" Kernel>"); printf("%s() ", __func__); printf( "Initializing timer structure: " "it's able to handle up to %d timers.", (15) ); printf("\r\n"); if(PRINTK_LEVEL_LOG == PRINTK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0)
                                                                    ;


    memset((void *) timerList, 0, sizeof(timerStruct) * (15));
}
...
unsigned char kernelRequestTimer(unsigned char *timerNoPtr)
{
    ...
    if (i == (15))
    {
        do { if( ( !g_xModemCommEnable ) && (PRINTK_LEVEL_ERROR) >= kernelGetDebugLevel() ) { if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" Kernel>"); printf("%s() ", __func__); printf( "Failed to request timer for task %d!", kernelGetTaskID() ); printf("\r\n"); if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0);
        return TASK_TIMER_FULL;
    }

    timerList[i].timerMask = (1<<0);
    timerList[i].timerTaskID = kernelGetTaskID();

    *timerNoPtr = i;

    do { if( ( !g_xModemCommEnable ) && (PRINTK_LEVEL_LOG) >= kernelGetDebugLevel() ) { if(PRINTK_LEVEL_LOG == PRINTK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" Kernel>"); printf("%s() ", __func__); printf( "Request timer %d for task %d.", i, kernelGetTaskID() ); printf("\r\n"); if(PRINTK_LEVEL_LOG == PRINTK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0);
...
unsigned char kernelEnableTimer(unsigned char timerNo, unsigned int interval, unsigned int event)
{
    if (timerNo >= (15))
    {
        do { if( ( !g_xModemCommEnable ) && (PRINTK_LEVEL_ERROR) >= kernelGetDebugLevel() ) { if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" Kernel>"); printf("%s() ", __func__); printf( "Invalid timer number %d!", timerNo ); printf("\r\n"); if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0);
        return TASK_TIMER_INVALID;
    }

    if (!(timerList[timerNo].timerMask & (1<<0)))
    {
        do { if( ( !g_xModemCommEnable ) && (PRINTK_LEVEL_ERROR) >= kernelGetDebugLevel() ) { if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[47;31m" );} printf(" Kernel>"); printf("%s() ", __func__); printf( "Timer %d isn't inited!", timerNo ); printf("\r\n"); if(PRINTK_LEVEL_ERROR == PRINTK_LEVEL_ERROR) {printf( "\033[0m" );} } } while (0);
    ...
    timerList[timerNo].timerMask |= (1<<1);
...
unsigned char kernelTimerIsEnable(unsigned char timerNo)
{
    ...
    if (timerList[timerNo].timerMask & (1<<1))
    {
...
int kernelShowTimer(unsigned int argc, unsigned char **argv)
{
    ...
    if (argc == 2 && argv[0][0] == '!')
    {
        do { if( !g_xModemCommEnable ) { printf( " %10s - show system timer info.\r\n", argv[1] ); if( shellRedirectBufSize > 1 ) { redirectLen = snprintf( (char *)shellRedirectBuf, shellRedirectBufSize , " %10s - show system timer info.\r\n", argv[1] ); if ( (redirectLen > 0) && ( redirectLen < shellRedirectBufSize ) ) { shellRedirectBufSize -= redirectLen; shellRedirectBuf += redirectLen; } else { shellRedirectBufSize = 0; } } } } while (0);
        return 0;
    }
    ...
    for (i = 0; i < (15); i++)
    {
        ...
        {
            taskID = timerList[i].timerTaskID;
            do { if( !g_xModemCommEnable ) { printf( " Timer %02d: task-%02d timing-%s \r\n", i, taskID, (timerList[i].timerMask & (1<<1)) ? "Yes" : "No" ); if( shellRedirectBufSize > 1 ) { redirectLen = snprintf( (char *)shellRedirectBuf, shellRedirectBufSize , " Timer %02d: task-%02d timing-%s \r\n", i, taskID, (timerList[i].timerMask & (1<<1)) ? "Yes" : "No" ); if ( (redirectLen > 0) && ( redirectLen < shellRedirectBufSize ) ) { shellRedirectBufSize -= redirectLen; shellRedirectBuf += redirectLen; } else { shellRedirectBufSize = 0; } } } } while (0)
                                                                                    ;
        }
    }
